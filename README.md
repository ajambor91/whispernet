...
### The Problem WhisperNet Solves
Secure communication is very difficult in our times. Many organizations try to track us, and some entities want to know our conversations. WhisperNet is a secure and anonymous communication platform. Its innovative infrastructure provides anonymity and secure messaging.

WhisperNet is designed for the highest level of security and anonymity. Chat is implemented using WebRTC, a peer-to-peer technology, meaning each message is sent directly to your friend. No messages are routed through our server, so we do not store any of your conversations. If someone were to attempt to access and analyze the server, they would only find random tokens used to facilitate connections between users. These tokens are stored in Redis using random UUIDs.

Each message is encrypted using AES256 in an isolated WebAssembly environment. WebAssembly allows binary code to run securely within a browser sandbox, which is isolated from JavaScript and the main browser environment. Data sanitization is also performed within WebAssembly. Cryptographic keys are randomly generated for every session on the server.

For the most secure conversations, you can verify your identity and verify the recipient's identity using PGP keys. To do this, you need to register on the site, provide your login, and send us your PGP public key file, or paste the raw key data. To verify your identity, you should sign in, provide your login, and then sign a random challenge generated by the server. You can paste your signature into a text area or download the challenge file, sign it, and upload the signed file.

Next, you can click "Force Signed Session". In this case, the application will ask your friend for their PGP key. They will have to verify their identity, and subsequently, both you and your friend will see each other's PGP keys. You can then choose to accept or reject the session based on this verification.

**NOTE:**
Your company or internet provider might be blocking P2P connections. In this case, you won't be able to connect with your friend. Some network routers (e.g., Symmetric NAT) may require specific configuration to work.  
...
- **UserSession (Spring Boot)**:  
  Handles user creation and sends messages to Session via Kafka. Optionally interacts with Redis for temporary data storage.
...
- **WebSocketApprovingServer (Node.js)**:
